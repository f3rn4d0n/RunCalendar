...
- **Taper que baja el volumen, no la intensidad** (`taperFactor`): en las 2 semanas previas a la
  ...
  penúltima semana afina menos que la de la carrera. Es lo que sostiene la evidencia (Bosquet et
  al.: volumen −41–60%, intensidad y frecuencia intactas, ~2 semanas), y bajar también el ritmo
  cuesta economía de carrera justo cuando hace falta. El aviso lo explica para que una semana corta
  no se lea como un error del plan.
...
- **Detalle de la sesión** (`WorkoutDetailView`): qué es, cómo se hace (series como "5 × 600 m
  ...
  ritmos exactos).
- **El día de series no es siempre la misma sesión** (`QualityEmphasis`). Rota por semana entre
  **cortas** (400–800 m, VO₂max), **largas** (1000–1600 m a umbral), **cuestas** (60 s, fuerza y
  economía) y **fartlek**; en las últimas 4 semanas antes de la meta todo pasa a **ritmo de
  carrera**, que es donde manda la especificidad. Antes solo existían repeticiones de 400–800 m
  salidas de una fórmula, así que quien entrenaba con la app nunca tocaba ni la velocidad pura ni
  el umbral en repeticiones. Comparte ancla con las descargas, así que el ciclo de calidad y el de
  carga van acompasados.
  **El fartlek va etiquetado con honestidad**: es el mismo estímulo que las cortas en otro
  envoltorio, y la tarjeta lo dice — está ahí para que apetezca salir, no porque adapte más.
  Cuestas y fartlek se miden **en tiempo** (`IntervalSpec.Rep`), no en metros: nadie corre "400 m"
  en un fartlek, y modelarlo así obligaría a inventar una distancia que nadie mide. El reloj
  traduce cada caso a su meta de WorkoutKit.
...
- **Entrevista** (`IntakeSheet`, `AthleteIntake`): tres preguntas que **ningún dato responde** — qué
  buscas, cuántos días *puedes* entrenar y si tienes dónde hacer cuestas. Más una cuarta, los km
  semanales, **solo si no hay historial en Salud**. La regla: los hechos del pasado se observan y no
  se preguntan; la intención y la capacidad no están en ningún sensor. Por eso no pregunta "de dónde
  vienes" (el techo por carga crónica ya lo codifica, y mejor: como número, no como etiqueta) ni el
  historial de lesiones (no hay forma calibrada de usar la respuesta).
  **No bloquea nada**: se ofrece desde *Ajustar* y la app funciona sin responderla.
...
- **Víspera de carrera protegida**: el día anterior a una carrera fija **nunca lleva calidad ni
  ...
  **descanso o 20–30 min muy suaves** sin prescribir uno: la fatiga de una sesión dura tarda
  ...
  corto antes de competir, así que la app no elige por ti. Es una regla de **colocación** dentro de
  la semana; el afinamiento de las 2 semanas previas es cosa aparte (ver *Taper* arriba).
...
- **Pruebas** (`RunCalendarTests`, Swift Testing): 174 pruebas en 16 suites, **en CI en cada PR**
  ...
  **Dobles de repositorio** en `RunCalendarTests/Doubles.swift`: uno por protocolo de Domain más un
  `TestApp` que arma los ViewModels igual que `AppContainer`. Con eso las pruebas llegan al
  **cableado** —qué sesiones alimentan el plan, cuándo se pausa la semana, qué se persiste al
  crear una meta—, que es donde las pruebas por caso de uso no alcanzan. Dos trampas: el stream de
  un doble debe **cerrar** tras emitir (si no, `start()` nunca vuelve) y las suites que tocan
  `UserDefaults` van `.serialized` y lo limpian al montar.
...
| **P1 · antes de tener usuarios** | **Observabilidad**: Crashlytics ✅ + no fatales ✅ (`Logger.failure`) + 5 eventos de uso ✅ (`Usage`) · **pruebas**: target ✅ + 174 pruebas ✅ + CI ✅; dobles de repositorio ✅ + cableado de ViewModels ✅; faltan HealthViewModel y TrainingViewModel |
